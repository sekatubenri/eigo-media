...
const categories = [
  { slug: 'school', label: '英会話スクール', desc: '通学型スクールを比較', emoji: '🏫' },
  { slug: 'online', label: 'オンライン英会話', desc: 'オンラインで手軽に学ぶ', emoji: '💻' },
  { slug: 'app', label: '英語アプリ', desc: 'スマホで英語を学ぶ', emoji: '📱' },
  { slug: 'beginner', label: '英語入門', desc: '英語学習の基礎知識', emoji: '🌱' },
]
export default function Home() {
  const articles = getAllArticles().slice(0, 6)
  return (
    <div>
      <section className="bg-gradient-to-r from-blue-600 to-indigo-500 text-white rounded-2xl p-8 mb-10 text-center">
        <h1 className="text-3xl font-bold mb-3">英語学習サービス徹底比較</h1>
        <p className="text-lg opacity-90">目的・レベル別に最適な英語学習法が見つかる</p>
      </section>
      <section className="mb-10">
        <h2 className="text-xl font-bold mb-4 text-gray-700">カテゴリから探す</h2>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {categories.map(c => (
            <Link key={c.slug} href={`/category/${c.slug}`} className="bg-white rounded-xl p-5 shadow hover:shadow-md transition text-center border border-blue-100">
