...
const SITE = { name: '英語学習ナビ', url: 'https://eigo-media.vercel.app' };
const AFFILIATE_TOP = `
<div style="background:#eff6ff;border:2px solid #2563eb;border-radius:8px;padding:16px;margin:24px 0;">
  <p style="font-weight:bold;color:#1e40af;margin:0 0 8px;">【PR】おすすめ英会話サービス</p>
  <ul style="list-style:none;padding:0;margin:0;display:flex;flex-direction:column;gap:8px;">
    <li><a href="https://px.a8.net/svt/ejp?a8mat=EIGO_PLACEHOLDER_1" rel="nofollow" style="display:inline-block;background:#2563eb;color:#fff;padding:10px 20px;border-radius:6px;text-decoration:none;font-weight:bold;">▶ DMM英会話（無料体験あり）</a></li>
    <li><a href="https://px.a8.net/svt/ejp?a8mat=EIGO_PLACEHOLDER_2" rel="nofollow" style="display:inline-block;background:#1d4ed8;color:#fff;padding:8px 16px;border-radius:6px;text-decoration:none;font-weight:bold;font-size:14px;">▶ ネイティブキャンプ（月額制・回数無制限）</a></li>
  </ul>
</div>`;
const AFFILIATE_BOTTOM = `
<div style="background:#fffbeb;border:2px solid #d97706;border-radius:8px;padding:16px;margin:24px 0;">
  <p style="font-weight:bold;color:#92400e;margin:0 0 12px;">📚 英語学習の参考書【Amazon】</p>
  <ul style="list-style:none;padding:0;margin:0;display:flex;flex-direction:column;gap:8px;">
    <li><a href="https://www.amazon.co.jp/s?k=%E8%8B%B1%E8%AA%9E%E5%AD%A6%E7%BF%92&linkCode=ll2&tag=mirainikibouw-22" rel="nofollow" target="_blank" style="color:#1d4ed8;text-decoration:underline;">▶ 英語学習本ランキング【Amazon】</a></li>
  </ul>
</div>`;
async function generateArticle() {
  ...
  const message = await client.messages.create({
    ...
    messages: [{ role: 'user', content: `あなたは英語学習メディア「${SITE.name}」の専門ライターです。以下のJSON形式のみで出力してください:\n{"title":"タイトル(40〜60文字)","description":"説明(120文字以内)","category":"${topic.category}","date":"${today}","content":"HTMLコンテンツ"}\nトピック:${topic.title}\ncontent要件:1500文字程度のHTML、h2×3〜5個、ul/table活用、具体的な料金・比較含む、JSON全体を必ず完結させる` }],
  });
